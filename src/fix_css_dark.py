...
for p in TARGETS:
    try:
        ...
        continue
    if "CSS = " not in s:
        print("SKIP (no CSS var):", p)
        continue
    # replace first CSS = """...""" block
    new_s, n = re.subn(r'CSS = """.*?"""', NEW_BLOCK, s, count=1, flags=re.DOTALL)
    if n == 0:
        print("SKIP (no triple-quote block matched):", p)
        continue
    # guard: ensure key dark token present
    if "#0B0F17" not in new_s:
        print("ERROR (dark token missing after replace):", p)
        continue
